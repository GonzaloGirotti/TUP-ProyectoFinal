...
export const createPesoHandler = async (
    ...
    try {
        ...
        if (!req.usuario) {
            return res.status(401).json({ message: 'No se encontró información de usuario en la sesión.' });
        }
    ...
    } catch (error: unknown) { // Usamos 'unknown' para ESlint
...
export const getPesosHandler = async (req: Request, res: Response) => {
    try {
        ...
        if (!req.usuario) {
            return res.status(401).json({ message: 'No se encontró información de usuario.' });
        }
...
export const deletePesoHandler = async (req: Request, res: Response) => {
    try {
        ...
        */
        ...
        if (!peso) {
            return res.status(404).json({ message: 'Registro de peso no encontrado o no autorizado.' });
        }
