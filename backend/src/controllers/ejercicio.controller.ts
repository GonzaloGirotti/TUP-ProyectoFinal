...
export const getEjerciciosHoyHandler = async (req: Request, res: Response) => {
  try {
    ...
    const total_calorias = ejercicios.reduce(
      (acc, curr) => acc + curr.calorias_quemadas,
      0,
    );

    return res.json({ total_calorias, ejercicios });
  } catch (error) {
    console.error("[EJERCICIO_CONTROLLER_GET]", error);
...
export const deleteEjercicioHandler = async (req: Request, res: Response) => {
  try {
    ...
    const registro = await Ejercicio.findOne({
      where: { id_ejercicio: id, id_usuario: req.usuario.id },
    });
    ...
    console.error("[EJERCICIO_CONTROLLER_DELETE]", error);
